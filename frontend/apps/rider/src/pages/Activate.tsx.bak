...
const Activate: React.FC = () => {
  ...
  const handleCheck = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const resp = await fetch(`${API_BASE}/auth/activate/check`, {
        method: 'POST',
  ...
  const handleActivate = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const resp = await fetch(`${API_BASE}/auth/activate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      // Force navigation to home; the ProtectedRoute will pick up the token
      navigate('/');
    } catch (err: any) {
      setError(err.message);
    } finally {
