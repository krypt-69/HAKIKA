...
async def update_phone(
    ...
    phone = body.phone
    """Allow a logged-in user to set their phone number."""
    # Basic Kenyan phone validation
    digits = ''.join(filter(str.isdigit, phone))
    if len(digits) < 9 or len(digits) > 12:
        raise HTTPException(status_code=400, detail="Invalid phone number format.")
    # Normalize: if starts with 0, convert to +254
    if digits.startswith('0') and len(digits) == 10:
        ...
        normalized = '+254' + digits[-9:]  # best effort
    from app.repositories.user_repository import UserRepository
    repo = UserRepository(service.user_repo.db)
    await repo.update_phone(current_user.id, normalized)
