...
class PaymentRepository:
    ...
    async def get_attempts(self, payment_id: uuid.UUID) -> list[PaymentAttempt]:
        ...
        return result.scalars().all()
    async def get_latest_final_payment(self, order_id: uuid.UUID):
        """Return the latest FINAL_PAYMENT for an order, or None."""
        from app.models.payment import PaymentType
        result = await self.db.execute(
            select(Payment)
            .where(Payment.order_id == order_id, Payment.payment_type == PaymentType.FINAL_PAYMENT)
