...
def test_full_financial_flow(client):
    email = f"fin-{uuid.uuid4()}@test.com"
    ...
    bid = bresp.json()["id"]
    presp = client.post(f"/api/v1/businesses/{bid}/products", json={"name": "FinItem", "original_price": 1000}, headers={"Authorization": f"Bearer {token}"})
    pid = presp.json()["id"]
    # rider
    rider_email = f"rider-{uuid.uuid4()}@test.com"
    client.post(f"/api/v1/riders/{bid}", json={"name": "R", "email": rider_email, "phone": "0700000003"}, headers={"Authorization": f"Bearer {token}"})
    ...
    oid = oresp.json()["id"]
    assert oresp.json()["total_amount"] == 1000.0
    # accept, assign, arrive, confirm
    ...
    assert any(s["amount"] == 980.0 for s in settlements)
    client.close()
